...
class KrakenPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    async def _update_trade_history(self):
        ...
        for trading_pair, resp in zip(self._trading_pairs, raw_responses):
            if not isinstance(resp, Exception):
                ...
                trade_entries = (
                    resp["result"]["trade_list"] if "trade_list" in resp["result"] else resp["result"]["data"]
                )
                if trade_entries:
    ...
    async def _request_order_status_data(self, tracked_order: InFlightOrder) -> Dict:
        ...
        query_params = {"symbol": exchange_symbol, "order_link_id": tracked_order.client_order_id}
    ...
    def _parse_trade_update(self, trade_msg: Dict, tracked_order: InFlightOrder) -> TradeUpdate:
        ...
        exec_time = (
            trade_msg["exec_time"] if "exec_time" in trade_msg else pd.Timestamp(trade_msg["trade_time"]).timestamp()
        )
    ...
    async def _set_trading_pair_leverage(self, trading_pair: str, leverage: int) -> Tuple[bool, str]:
        ...
        success = False
        msg = ""
        if resp["ret_code"] == CONSTANTS.RET_CODE_OK or (
            resp["ret_code"] == CONSTANTS.RET_CODE_LEVERAGE_NOT_MODIFIED and resp["ret_msg"] == "leverage not modified"
        ):
            success = True
        else:
            formatted_ret_code = self._format_ret_code_for_print(resp["ret_code"])
