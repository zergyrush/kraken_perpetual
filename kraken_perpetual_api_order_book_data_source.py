...
if TYPE_CHECKING:
    from hummingbot.connector.derivative.kraken_perpetual.kraken_perpetual_derivative import (
        KrakenPerpetualDerivative,
    )
...
class KrakenPerpetualAPIOrderBookDataSource(PerpetualAPIOrderBookDataSource):
    ...
    async def _parse_order_book_diff_message(
        ...
        if event_type == "delta":
            ...
            trading_pair = (
                await self._connector.trading_pair_associated_to_exchange_symbol(symbol)
            )
            ...
            for entry in entries:
                ...
                if "next_funding_time" in entry:
                    info_update.next_funding_utc_timestamp = int(
                        pd.Timestamp(
                            str(entry["next_funding_time"]), tz="UTC"
                        ).timestamp()
                    )
                if "predicted_funding_rate_e6" in entry:
                    info_update.rate = Decimal(
                        str(entry["predicted_funding_rate_e6"])
                    ) * Decimal(1e-6)
    ...
    async def _request_complete_funding_info(self, trading_pair: str):
        ...
        url_predicted = web_utils.get_rest_url_for_endpoint(
            endpoint=endpoint_predicted, trading_pair=trading_pair, domain=self._domain
        )
        limit_id_predicted = web_utils.get_rest_api_limit_id_for_endpoint(
            endpoint_predicted, trading_pair
        )
        tasks.append(
            rest_assistant.execute_request(
                url=url_predicted,
    ...
    def _get_bids_and_asks_from_rest_msg_data(
        ...
        for i, row in enumerate(snapshot):
            if row["side"] == "Sell":
                ...
                break
        bids = [
            (float(row["price"]), float(row["size"])) for row in snapshot[:bisect_idx]
        ]
        asks = [
            (float(row["price"]), float(row["size"])) for row in snapshot[bisect_idx:]
        ]
