            ...
            if linear_trading_pairs:
                tasks.append(
                    self._listen_for_subscriptions_on_url(
                        url=web_utils.wss_linear_public_url(self._domain), trading_pairs=linear_trading_pairs
                    )
                )
            if non_linear_trading_pairs:
                tasks.append(
                    self._listen_for_subscriptions_on_url(
                        url=web_utils.wss_non_linear_public_url(self._domain), trading_pairs=non_linear_trading_pairs
                    )
                )
        ...
        if event_type == "delta":
            ...
            for entry in entries:
                ...
                if "predicted_funding_rate_e6" in entry:
                    info_update.rate = Decimal(str(entry["predicted_funding_rate_e6"])) * Decimal(1e-6)
    ...
    def _get_bids_and_asks_from_rest_msg_data(
        ...
        for i, row in enumerate(snapshot):
            if row["side"] == "Sell":
                bisect_idx = i
                break
        bids = [(float(row["price"]), float(row["size"])) for row in snapshot[:bisect_idx]]
        asks = [(float(row["price"]), float(row["size"])) for row in snapshot[bisect_idx:]]
        return bids, asks
